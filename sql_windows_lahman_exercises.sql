...
SELECT
	...
	w,
	divid,
	rank() OVER (PARTITION BY yearid, divid ORDER BY w ASC ) AS RANK  
FROM teams
)
SELECT 
	teamid,
	count(*) AS times_last_place
FROM last_place 
WHERE RANK = 1
GROUP BY teamid 
ORDER BY times_last_place DESC; 
...
1. mumber OF seasons played
2. total career home runs per season played
